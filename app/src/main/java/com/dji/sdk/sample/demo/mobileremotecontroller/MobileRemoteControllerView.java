...
public class MobileRemoteControllerView extends RelativeLayout
    ...
    public void onClick(View v) {
        ...
        TimerTask timerTaskObj = new TimerTask() {
            public void run() {
                ...
                Float altitude=flightController.getState().getAircraftLocation().getAltitude();

                if(flightController.getState().areMotorsOn()){
                    postGpsData(latitude, longitude, altitude);
                }
            }
        };
    ...
    public void postGpsData(final Double latitude, final Double longitude, final Float altitude) {
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                try {
                    URL url = new URL("http://46.101.52.245:3000/GPSDATA");
